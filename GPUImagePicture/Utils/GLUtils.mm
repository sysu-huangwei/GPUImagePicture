...
+ (GLuint) LoadFileToTexture:(NSString *) path {
    ...
    GLuint texture = [self LoadUIImageToTexture:image];
    image = nil;
    return texture;
}
...
+ (GLuint) LoadUIImageToTexture:(UIImage *) image {
    if (image != nil) {
        unsigned char* data = [self RGBADataWithAlpha:image];
        GLuint texture = [self LoadTexture_BYTE:data width:image.size.width height:image.size.height glFormat:GL_RGBA];
        free(data);
        return texture;
    }
    return 0;
